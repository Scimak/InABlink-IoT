...
This file represents a code snippet that runs inside the LightingActivity.java in the mobile application. The code runs when a lighting selection is made. The lighting selection is stored in the selected variable, and depeding on which selection is chosen, the path taken on the ESP32 server is different. Volley library is used to make the HTTP request to the ESP32, and the ESP32 takes that request and performs it.
